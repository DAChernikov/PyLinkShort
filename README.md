...
В результате запустится Docker-приложение с тестированием API. По окончанию тестирования, в консоль будет выведен результат. Пример результата тестирования приведен в файле `coverage.html` в корне репозитория (или на картинке `Отчет о покрытии тестами.png`).
